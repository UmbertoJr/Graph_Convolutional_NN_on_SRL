...
with G.as_default():
    ...
    with tf.name_scope("Semantic_Role_Labeling-system"):
        with tf.name_scope("placeholders"):
            with tf.name_scope("mask"):
                ...
                with tf.name_scope("final_mask"):
                    final_mask = mask_for_sequences & mask_for_predicates
        ...
        with tf.name_scope("inputs"):
            ...
            with tf.name_scope("concatenated_input"):
                ...
                # final shape is [batch*max_len_sent*max_pred_in_sent, concat_dim_encode]
    ...
    validation_metrics_vars = tf.get_collection(tf.GraphKeys.LOCAL_VARIABLES, 
                                                scope='Semantic_Role_Labeling-system/metrics_evaluation/')
    ...
    with tf.name_scope("trainer"):
        ...
        with tf.Session(config=tf.ConfigProto(log_device_placement=True)) as sess:
            ...
            for h in range(0,1000):
                ...
                X_emb, X_POS, X_LEMMA, Lapl_i, Lapl_v, Sp_i, Sp_v, Pred_Ind, Arg_pos, Seq_Len, Pred_count = gen(batch_size)
                ...
                if h%5 == 0:
                    ...
                    t_, l_srl , acc_, sum_model = sess.run([train_op, loss_srl,accuracy_srl_op, merged],
                    ...
                    X_emb_eval, X_POS_eval, X_LEMMA_eval, Lapl_i_eval, Lapl_v_eval, Sp_i_eval,Sp_v_eval, Pred_Ind_eval, Arg_pos_eval, Seq_Len_eval, Pred_count_eval = gen_eval(batch_size)
                    ...
                    l_srl_eval , acc_eval, sum_model_eval = sess.run([loss_srl, accuracy_srl_op, merged],feed_dict=feed_dict_eval,options=run_options,run_metadata=run_metadata) 
                ...
                if h > 80 and h%30==0: #
                    l_srl_eval , acc_eval, preds_srl, sum_model_eval = sess.run([loss_srl, accuracy_srl_op, predictions_srl, merged],feed_dict=feed_dict_eval,options=run_options,run_metadata=run_metadata) 
                    ...
                    for it in range(4):
                        ...
                        semantic_vec_test = sess.run([predictions_srl],feed_dict=feed_dict_test)
                
                        np.save("../../data/semantic-role-labeling/Cud/srl_"+str(it)+"_test1.npy", semantic_vec_test)
                        ...
                        semantic_vec_test2 = sess.run([predictions_srl],feed_dict=feed_dict_test2)
                        
                        np.save("../../data/semantic-role-labeling/Cud/srl_"+str(it)+"_test2.npy", semantic_vec_test2)
