...
with G.as_default():
    with tf.name_scope("inputs"):
        ...
        with tf.name_scope("concat_inputs"):
            ...
            inputs = tf.concat([embeddings, POS_emb, lemma_emb, one_hot_POS ], axis = -1)
    ...
    with tf.name_scope("Semantic_Role_Labeling-system"):
        ...
        with tf.name_scope("inputs"):
            with tf.name_scope("position_predicate"):
                predicate_encode = tf.gather_nd(last_layer, indices_for_predicate)
            ...
            with tf.name_scope("GCN_encode_reshape_for_concatenatio"):
                output_repeated = tf.reshape( tf.tile(last_layer,[1,max_pred_in_sent, 1]),
                                             shape=[batch_size, max_pred_in_sent*max_length_sentences, input_dim_feat])
                # repeat each sentence max_pred_in_sent times
                
                
            with tf.name_scope("concatenated_input"):
                input_to_srl_system_flat = tf.reshape(tf.concat([pred_repeated,output_repeated], axis=2) , [-1, 2*input_dim_feat])
        ...
        with tf.name_scope("loss_with_mask"):
            ...
            with tf.name_scope("mask_only_arguments"):
                         losses_after_mask_sent_and_pred = tf.boolean_mask(losses_srl, mask_seq_and_pred)
                  
            loss_srl_args = tf.reduce_mean(losses_after_mask_sent_and_pred)
    ...
    with tf.name_scope("detection-system"):
        ...
        with tf.name_scope("is_arg"):
            ...
            arg_detection_after_mask_seq = tf.boolean_mask(arg_detection, mask_seq_and_pred)
        
        with tf.name_scope("detection"):
            detection_srl_flat_logit = input_to_srl_system_flat @ w_srl_det + bias_srl_det
        ...
        with tf.name_scope("loss_with_mask"):
            ...
            with tf.name_scope("mask_only_sentence_and_possible_predicates"):
                         losses_after_mask_seq = tf.boolean_mask(losses_detection, mask_seq_and_pred)
            
            loss_detection = tf.reduce_mean(losses_after_mask_seq)
    ...
    with tf.name_scope("metrics_evaluation"):
        ...
        accuracy_det, accuracy_det_op = tf.metrics.accuracy(labels= arg_detection_after_mask_seq,
                                                            predictions = tf.boolean_mask(tf.argmax(detection_srl_logit_nice, axis=3),
                                                                                          mask_seq_and_pred))
    ...
    with tf.name_scope("Optimizer") as scope:
        ...
        final_loss = loss_srl     +  1000*loss_detection # + 100*loss_srl_args 
